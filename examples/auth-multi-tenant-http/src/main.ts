...
const bootstrap = async () => {
  ...
  appLogger.info('Auth multi-tenant HTTP example ready', {
    url: `http://${displayHost}:${port}`,
    authEndpoints: `http://${displayHost}:${port}${httpOptions.prefix}/*`,
  ...
  const shutdown = async (signal: string) => {
    ...
    await app.close();
    appLogger.info('Auth multi-tenant HTTP example stopped');
    process.exit(0);
  };
...
bootstrap().catch((error) => {
  const fallbackLogger = new Logger({ context: 'AuthMultiTenantHttpExample' });
  fallbackLogger.fatal('Failed to start the auth multi-tenant HTTP example', error instanceof Error ? error : undefined);
  process.exit(1);
});
