...
const configSnippet = `# config/serialization.yaml
serialization:
  enabled: true
...
const httpSerialization = `import { Module, ConfigService } from '@nl-framework/core';
...
const graphqlSerialization = `import { Module, ConfigService } from '@nl-framework/core';
import { ConfigModule } from '@nl-framework/config';
...
const manualSerialize = `import { serialize, SerializeOptions } from '@nl-framework/core';
...
export default function SerializationTechniquesPage() {
  return (
    <article className="space-y-8">
      <div className="space-y-3">
        ...
        <p className="text-lg text-muted-foreground">
          Shape your JSON responses with class-transformer. Nael provides a shared <code>serialize()</code> helper,
          a <code>@SerializeOptions()</code> decorator for per-handler control, and HTTP/GraphQL interceptors that respect DI config.
          Streams, buffers, and <code>Response</code> objects are passed through unchanged.
        </p>
      </div>
      ...
      <section className="space-y-4" id="config">
        ...
        <p className="text-muted-foreground">
          Keep serialization defaults in <code>ConfigModule</code> so interceptors can read groups or strategies without hardcoding.
      ...
      <section className="space-y-4" id="http">
        ...
        <p className="text-muted-foreground">
          Attach <code>HttpSerializationInterceptor</code> globally or per-controller. Combine with <code>@SerializeOptions()</code>
          to set <code>class-transformer</code> groups or naming strategies per handler.
        </p>
      ...
      <section className="space-y-4" id="graphql">
        ...
        <p className="text-muted-foreground">
          Wrap resolvers with <code>GraphqlSerializationInterceptor</code> to convert DTO instances into plain objects before they hit Apollo.
      ...
      <section className="space-y-4" id="manual">
        ...
        <p className="text-muted-foreground">
          Use <code>serialize()</code> directly in jobs or microservices when you need the same rules outside HTTP/GraphQL.
          Decorate classes with <code>@SerializeOptions()</code> to keep group-aware transforms close to the DTO.
        </p>
