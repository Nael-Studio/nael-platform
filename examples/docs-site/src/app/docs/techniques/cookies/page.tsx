...
const readCookies = `import { Controller, Get, Req } from '@nl-framework/http';
...
const setCookie = `import { Controller, Post } from '@nl-framework/http';
...
const clearCookie = `// Overwrite with expired cookie
const expireCookie = 'session_id=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0';
return new Response(JSON.stringify({ ok: true }), {
  headers: { 'set-cookie': expireCookie },
});`;

const multiTenant = `// With Better Auth multi-tenant loader, set a per-tenant prefix (e.g., mt-acme.session_token)
...
export default function CookiesTechniquesPage() {
  return (
    <article className="space-y-8">
      <div className="space-y-3">
        ...
        <p className="text-lg text-muted-foreground">
          Use the native <code>Request</code>/<code>Response</code> APIs to manage cookies in Nael HTTP handlers. Parse
      ...
      <section className="space-y-4" id="read">
        ...
        <p className="text-muted-foreground">
          Access cookies via <code>req.headers.get('cookie')</code>. Parse into a map for easy lookup:
        </p>
      ...
      <section className="space-y-4" id="write">
        ...
        <p className="text-muted-foreground">
          Write <code>Set-Cookie</code> headers directly. Always include <code>HttpOnly</code>, <code>Secure</code>,
          and an appropriate <code>SameSite</code> value (<code>Lax</code> for most API cookies; <code>None</code> when shared across origins with <code>Secure</code>).
        </p>
      ...
      <section className="space-y-4" id="multi-tenant">
        ...
        <p className="text-muted-foreground">
          Better Auth supports per-tenant cookie prefixes. Set <code>cookiePrefix</code> in the tenant resolver to keep
