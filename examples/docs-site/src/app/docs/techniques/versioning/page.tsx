...
const configSnippet = `# config/http.yaml
http:
  port: 3000
  versioning:
    enabled: true
...
const bootstrap = `import { createHttpApplication } from '@nl-framework/http';
...
const controllerExample = `import { Controller, Get, Version } from '@nl-framework/http';
...
const calling = `# URI strategy (/v1 prefix)
...
export default function VersioningTechniquesPage() {
  return (
    <article className="space-y-8">
      ...
      <section className="space-y-4" id="config">
        ...
        <p className="text-muted-foreground">
          ...
          negotiated version. URI strategy strips a prefix like <code>/v1</code>; header strategy reads <code>x-api-version</code>;
          media strategy looks for <code>v</code> in <code>Accept</code>.
        </p>
      ...
      <section className="space-y-4" id="decorators">
        ...
        <p className="text-muted-foreground">
          Use <code>@Version()</code> on controllers for defaults and on handlers for overrides. The router picks the
      ...
      <section className="space-y-4" id="calling">
        ...
        <p className="text-muted-foreground">
          With URI strategy, prefix routes (e.g., <code>/v1/users</code>). With header/media strategies, keep URLs stable
