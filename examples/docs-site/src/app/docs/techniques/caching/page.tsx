...
const configSnippet = `# config/cache.yaml
cache:
  ttlMs: 60000
  prefix: api
  redisUrl: \${REDIS_URL}
...
const diHttp = `import Redis from 'ioredis';
import { Module, ConfigService, Inject } from '@nl-framework/core';
import { buildCacheKey, InMemoryCacheStore, RedisCacheStore, type CacheStore } from '@nl-framework/core';
import { ConfigModule } from '@nl-framework/config';
import { Controller, Get, UseInterceptors, HttpCacheInterceptor } from '@nl-framework/http';
...
const graphqlCaching = `import { Module, ConfigService } from '@nl-framework/core';
import { buildCacheKey, InMemoryCacheStore } from '@nl-framework/core';
import { ConfigModule } from '@nl-framework/config';
...
const redisSnippet = `import Redis from 'ioredis';
import { Module, ConfigService } from '@nl-framework/core';
import { RedisCacheStore } from '@nl-framework/core';
import { ConfigModule } from '@nl-framework/config';
import { Controller, Get, UseInterceptors, HttpCacheInterceptor } from '@nl-framework/http';
...
const manualCaching = `import { buildCacheKey, InMemoryCacheStore } from '@nl-framework/core';
...
export default function CachingTechniquesPage() {
  return (
    <article className="space-y-8">
      <div className="space-y-3">
        ...
        <p className="text-lg text-muted-foreground">
          Cache expensive responses without coupling your handlers to storage details. Nael ships a CacheStore interface,
          an in-memory LRU implementation, Redis adapter, and cache interceptors for both HTTP and GraphQL pipelines.
          Keys are stable via <code>buildCacheKey()</code>, TTLs are millisecond-based, and you stay in control of what
          gets cached.
        </p>
      </div>
      ...
      <section className="space-y-4" id="stores">
        ...
        <p className="text-muted-foreground">
          Drive cache settings from <code>ConfigModule</code> and expose stores/interceptors as providers. The examples
          below use Symbols so <code>@UseInterceptors()</code> can resolve instances from the container and still fall back
      ...
      <section className="space-y-4" id="http">
        ...
        <p className="text-muted-foreground">
          ...
          TTLs, and cacheable status codes in the factory. <code>HttpCacheInterceptor</code> caches GET/HEAD responses by
          default and replays them as normal <code>Response</code> objects.
        </p>
        ...
        <div className="rounded-lg border border-sky-500/40 bg-sky-500/10 p-4 text-sm text-sky-800 dark:text-sky-100">
          <strong>Tip:</strong> Use <code>shouldCacheResult</code> to skip caching on auth/tenant mismatches or when response headers
      ...
      <section className="space-y-4" id="redis">
        ...
        <p className="text-muted-foreground">
          Swap the store provider for <code>RedisCacheStore</code> to share cache across workers. Prefix keys per app or tenant,
      ...
      <section className="space-y-4" id="manual">
        ...
        <p className="text-muted-foreground">
          Use <code>buildCacheKey()</code> with primitives and objects to generate stable keys, then call the store directly in
