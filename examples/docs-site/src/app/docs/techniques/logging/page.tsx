...
const configureLogger = `import { NaelFactory } from '@nl-framework/platform';
...
const app = await NaelFactory.create(AppModule, { logger: appLogger });
await app.listen({ http: 3000 });`;

const requestLogging = `import { MiddlewareHandler } from '@nl-framework/http';
...
const childLoggers = `import { Injectable } from '@nl-framework/core';
import { LoggerFactory } from '@nl-framework/logger';
...
const customTransport = `import { Logger, type LogMessage, type LoggerTransport } from '@nl-framework/logger';
...
const elasticApm = `import apm from 'elastic-apm-node';
...
export default function LoggingTechniquesPage() {
  return (
    <article className="space-y-8">
      <div className="space-y-3">
        ...
        <p className="text-lg text-muted-foreground">
          Emit structured logs across HTTP handlers, schedulers, and microservices. Nael&apos;s logger ships with JSON output,
      ...
      <section className="space-y-4" id="bootstrap">
        ...
        <p className="text-muted-foreground">
          Provide a root <code>Logger</code> when bootstrapping your app to set the service name and log level. Downstream
      ...
      <section className="space-y-4" id="http">
        ...
        <p className="text-muted-foreground">
          Add an HTTP middleware that logs method, path, status, and latency. Use <code>LoggerFactory</code> to keep
      ...
      <section className="space-y-4" id="di">
        ...
        <p className="text-muted-foreground">
          Inject <code>LoggerFactory</code> into services and create child loggers per operation or tenant. Child loggers
      ...
      <section className="space-y-4" id="transports">
        ...
        <p className="text-muted-foreground">
          The default console transport prints JSON to stdout. Implement <code>LoggerTransport</code> to forward logs to
