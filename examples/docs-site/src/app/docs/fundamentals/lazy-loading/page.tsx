...
const lazyLoaderSnippet = `import { LazyModuleLoader } from '@nl-framework/core';
...
const listenerSnippet = `import { Application } from '@nl-framework/core';
...
const onDemandResolverSnippet = `import { LazyModuleLoader } from '@nl-framework/core';
  ...
  return (
    <article className="space-y-8">
      ...
      <section className="space-y-4" id="tenant-use-case">
        ...
        <p className="text-muted-foreground">
          Tie the loader into your tenant resolution logic (guards, middleware, schedulers) and call <code>loader.load(PremiumReportsModule)</code>
          only when <code>tenant.plan === 'premium'</code>. This keeps memory predictable and shortens boot time for the common case without removing
      ...
      <section className="space-y-4" id="loader">
        ...
        <p className="text-muted-foreground">
          The <code>LazyModuleLoader</code> is injectable from any application context (core, HTTP, GraphQL, schedulers). Call <code>load()</code> with
        ...
        <p className="text-sm text-muted-foreground">
          The loader understands default exports as well, so returning the raw <code>import()</code> promise works if your module file exports a
          <code>default</code> class.
        </p>
      </section>
      ...
      <section className="space-y-4" id="listeners">
        ...
        <p className="text-muted-foreground">
          The <code>ApplicationContext</code> now exposes <code>loadModule()</code> and <code>addModuleLoadListener()</code>. Use them to wire up your
