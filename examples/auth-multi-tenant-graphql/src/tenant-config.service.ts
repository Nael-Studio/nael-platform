...
const DEFAULT_TENANTS: Record<string, TenantRecord> = {
  default: {
    ...
    dbName: 'nael-mt-graphql-default',
    cookiePrefix: 'mtg-default',
  },
  acme: {
    secretEnv: 'BETTER_AUTH_SECRET_ACME',
    dbName: 'nael-mt-graphql-acme',
    cookiePrefix: 'mtg-acme',
  },
};
  ...
  async load(tenantKey: string, db: Db): Promise<BetterAuthModuleOptions> {
    ...
    return {
      betterAuth: {
        ...
        advanced: { cookiePrefix: tenant.cookiePrefix ?? `mtg-${tenantKey}` },
      },
