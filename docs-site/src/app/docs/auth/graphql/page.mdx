...
import { UseGuards, CurrentSession } from '@nl-framework/auth';
import { BetterAuthGuard, BetterAuthSession } from '@nl-framework/auth';
...
export const contextNote = `// When bootstrapping NL Framework GraphQL:
...
export const graphqlBootstrap = `import { NL FrameworkFactory } from '@nl-framework/platform';
...
<article className="space-y-8">
  ...
  <section className="space-y-4" id="guard">
    ...
    <div className="text-muted-foreground">
      ...
      details by passing the userId/tenant down.
    </div>
