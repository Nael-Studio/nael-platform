---
title: 'Module reference · NL Framework'
...
<article className="space-y-8">
      ...
      <section className="space-y-4" id="resolve">
        ...
        <div className="text-muted-foreground">
          Call `moduleRef.resolve(Token)` to retrieve a provider using the same injection graph NL Framework builds at bootstrap. The promise resolves
      ...
      <section className="space-y-4" id="transient">
        ...
        <div className="text-muted-foreground">
          Use `moduleRef.create()` when you need a throwaway instance that should not be cached by the container. NL Framework will construct the
