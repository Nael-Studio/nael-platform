---
title: 'Gateways · GraphQL'
description: 'Run an Apollo Federation gateway inside Nael using the built-in FederationGatewayApplication.'
---
...
export const gatewayOptions = `import { NaelFactory } from '@nl-framework/platform';
...
export const integratedGateway = `// If you also start HTTP, Nael mounts the gateway handler into the HTTP server:
...
<article className="space-y-8">
  <div className="space-y-3">
    ...
    <div className="text-lg text-muted-foreground max-w-3xl">
      Nael ships a built-in Apollo Federation gateway. Pass subgraph URLs to <code>NaelFactory</code>, and it boots an
  ...
  <section className="space-y-4" id="requirements">
    ...
    <ul className="list-disc space-y-2 pl-5 text-muted-foreground">
      <li>Each Nael service must enable <code>federation: true</code> in <code>GraphQLModule</code>.</li>
